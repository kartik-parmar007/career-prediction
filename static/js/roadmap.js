/**
 * AI Personalized Roadmap Manager (Clean White Theme)
...
function renderRoadmapData(roadmap) {
  ...
  if (sourceBadgeEl) {
    if (roadmap.generated_by === 'live_ai') {
      sourceBadgeEl.innerHTML = `<i class="fa-solid fa-sparkles text-cyan-600"></i> AI Generated`;
    } else {
      sourceBadgeEl.innerHTML = `<i class="fa-solid fa-microchip text-indigo-600"></i> AI Engine`;
    }
  }
  ...
  roadmap.phases.forEach((p, idx) => {
    ...
    phasesHtml += `
      <div class="glass-card p-5 sm:p-7 relative overflow-hidden transition-all duration-300 hover:border-indigo-300 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-2.5 mb-3.5">
          <div class="flex items-center gap-2.5">
            <span class="w-8 h-8 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-700 font-bold flex items-center justify-center text-xs sm:text-sm shadow-sm">
              0${phaseNum}
            </span>
            <h3 class="text-base sm:text-lg font-bold text-slate-900">${p.title}</h3>
          </div>
          <div class="flex items-center gap-1.5">
            <span class="px-2.5 py-0.5 rounded-full text-[11px] font-semibold border ${isPriority ? 'bg-amber-50 border-amber-300 text-amber-800' : 'bg-indigo-50 border-indigo-200 text-indigo-700'}">
              ${focusTag}
            </span>
        ...
        <div class="mb-4">
          <h4 class="text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">Core Skills</h4>
          <div class="flex flex-wrap gap-1 mb-2.5">
            ${(p.skills || []).map(s => `<span class="px-2 py-0.5 rounded-md bg-slate-100 border border-slate-200 text-slate-700 text-[11px] font-medium">${s}</span>`).join('')}
          </div>
          <ul class="text-xs text-slate-600 space-y-1 pl-4 list-disc marker:text-indigo-600">
            ${(p.topics || []).map(t => `<li>${t}</li>`).join('')}
          </ul>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-2 pt-3 border-t border-slate-100">
          <div class="p-3 rounded-xl bg-slate-50 border border-slate-200">
            <h5 class="text-[11px] font-bold text-indigo-700 uppercase tracking-wider mb-2 flex items-center gap-1">
            ...
            <ul class="text-xs text-slate-700 space-y-1.5">
              ${(p.practice_tasks || []).map((task, tIdx) => `
                <li class="flex items-start gap-2">
                  <input type="checkbox" id="task-${phaseNum}-${tIdx}" class="mt-0.5 w-3.5 h-3.5 rounded text-indigo-600 focus:ring-0 bg-white border-slate-300 cursor-pointer" onchange="toggleTaskCheck('task-${phaseNum}-${tIdx}')">
          ...
          <div class="p-3 rounded-xl bg-slate-50 border border-slate-200">
            <h5 class="text-[11px] font-bold text-emerald-700 uppercase tracking-wider mb-1.5 flex items-center gap-1">
              <i class="fa-solid fa-folder-open text-[10px]"></i> Milestone Capstone
            </h5>
            <p class="text-xs text-slate-800 font-semibold mb-1">${p.project}</p>
            <p class="text-xs text-slate-500 leading-relaxed"><strong class="text-slate-700">Milestone:</strong> ${p.milestone}</p>
          </div>
        </div>
      </div>
    `;
